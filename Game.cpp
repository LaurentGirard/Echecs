...
int main()
{
	...
	for(int ww = 0; ww< playerA.getPieces()[10]->getmovements().size() ; ++ww)
	{
		for(int www = 0; www< playerA.getPieces()[10]->getmovements()[ww].size() ; ++www) 		
		std::cout << playerA.getPieces()[10]->getmovements()[ww][www].getX()<<" - "<<playerA.getPieces()[10]->getmovements()[ww][www].getY() << std::endl;
		
		std::cout << playerA.getPieces()[10]->getmovements()[ww].size()<< std::endl;
	};
	return 0;
}
